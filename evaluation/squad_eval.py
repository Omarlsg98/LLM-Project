...
def get_raw_scores_mod(dataset, preds):
    exact_scores = {}
    f1_scores = {}
    for item in dataset:
        qid = item["id"]
        gold_answers = [
            a["text"] for a in item.get("answers", []) if normalize_answer(a["text"])
        ]
        if not gold_answers:
            # For unanswerable questions, only correct answer is empty string
            gold_answers = [""]
...
def main(data_file, preds, na_probs, na_prob_thresh, image_dir=None, out_file=None):
    with open(data_file) as f:
        dataset_json = json.load(f)
        dataset = dataset_json["data"]

    na_probs_provided = na_probs is not None
    if not na_probs_provided:
    ...
    if na_probs_provided:
        find_all_best_thresh(
            out_eval, preds, exact_raw, f1_raw, na_probs, qid_to_has_ans
        )
    if na_probs_provided and image_dir:
        run_precision_recall_analysis(
            out_eval, exact_raw, f1_raw, na_probs, qid_to_has_ans, image_dir
        )
        histogram_na_prob(na_probs, has_ans_qids, image_dir, "hasAns")
        histogram_na_prob(na_probs, no_ans_qids, image_dir, "noAns")
    if out_file:
